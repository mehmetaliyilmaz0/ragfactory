...
class TestUnsupportedAdvancedGeneration:
    ...
    def test_agentic_enabled_is_unsupported_error(self) -> None:
        result = validate(_cfg(
            generation=GenerationConfig(
                llm=OpenAILLMConfig(),
        ...
        assert result.valid is False
        assert _has_error(result, "UNSUPPORTED_ADVANCED_AGENTIC")
...
class TestAdvancedGenerationDisabledStates:
    def test_all_three_enabled_emit_three_unsupported_errors(self) -> None:
        result = validate(_cfg(
            generation=GenerationConfig(
                llm=OpenAILLMConfig(),
